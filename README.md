...
`.github/workflows/release-desktop.yml` 会依次构建 macOS Apple Silicon（`aarch64-apple-darwin`）和 Windows x64（`x86_64-pc-windows-msvc`）版本。推送与 `package.json` 版本一致的标签即可发布，例如版本为 `0.2.0` 时执行：
...
发布流程会先创建草稿 Release，上传 DMG、Windows NSIS 安装包、两个平台的 updater 文件及签名，并合并生成一个包含 macOS 和 Windows 更新条目的 `latest.json`；两个平台构建都成功后才会正式发布。桌面应用启动后会自动检查 GitHub 最新 Release，也可以点击顶部的更新图标手动检查。
